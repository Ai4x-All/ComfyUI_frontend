<template>
  ...
  >
    <div class="actionbar-content flex items-center select-none" ref="panelRef">
      ...
      <ComfyQueueButton ref="queueButtonRef" />
    </div>
  </div>
</template>
...
import ExecuteIcon from "@/assets/images/icon_execute.png"
...
const queueButtonRef = ref(null)
const dragHandleRef = ref(null)
const showDragHandle = ref(false)
const isDocked = useLocalStorage('Comfy.MenuPosition.Docked', false)
const storedPosition = useLocalStorage('Comfy.MenuPosition.Floating', { x: 0, y: 0 })
...
/*watchEffect(() => {
  dragHandleRef.value = queueButtonRef.value ?.getDragHandle()
})*/
// 或
onMounted(() => {
  dragHandleRef.value = queueButtonRef.value ?.getDragHandle()
